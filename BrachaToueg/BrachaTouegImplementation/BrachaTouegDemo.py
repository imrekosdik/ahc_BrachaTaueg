...
class BrachaTouegComponentModel(GenericModel):
    ...
    def terminate_snapshot(self):
        ...
        if self.deadlock_detection_initiator:
            message = GenericMessage(GenericMessageHeader(LaiYangMessageTypes.TERMINATED, self.componentinstancenumber, self.componentinstancenumber), None)
            self.send_self(Event(self, EventTypes.MFRT, message)) 
    ...
    def on_receiving_done(self, eventobj: Event):
        ...
        if self.number_of_received_done_messages == len(self.outgoing_channels):
            if self.deadlock_detection_initiator:
                if self.free:
