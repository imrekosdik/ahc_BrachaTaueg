...
class ShavitFrancezComponentModel(GenericModel):
    ...
    def on_receiving_become_active(self, eventobj: Event):
        ...
        '''
        logger.critical(f"{self.componentname}.{self.componentinstancenumber} became active")
        if not self.is_active:
            self.is_active = True
        self.send_basic_message()
    ...
    def on_receiving_become_passive(self, eventobj: Event):
        ...
        if self.is_active:
            self.is_active = False
        logger.critical(logger.info(f"{self.componentname}.{self.componentinstancenumber} calls leave tree on becoming passive"))
        self.leave_tree()
    ...
    def on_receiving_start_wave(self, eventobj: Event):
        ...
        if self.is_active == False and self.number_of_children == 0: 
            logger.info(self.wave_parent)
            self.number_of_received_wave_messages += 1
            logger.info(self.number_of_received_wave_messages)
    ...
    def leave_tree(self):
        ...
        if not self.is_active and self.number_of_children == 0:
            ...
            if self.termination_parent is not None and self.termination_parent is not self.componentinstancenumber:
                logger.info(f"{self.componentname}.{self.componentinstancenumber} leaves its parent")
